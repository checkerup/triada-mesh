...
## 自主三合一会诊技能 (`triada-council`)

Triada Mesh 在 [`skills/triada-council/`](skills/triada-council/) 中提供开箱即用的跨环境技能：
- **自然语言自动触发**: 当输入包含 `консилиум триады`、`триада реши`、`запусти консилиум` 或 `/triada-council` 时自动激活。
- **跨会话历史上下文挖掘**: 传入历史会话 ID（如 `20260923_023804_05b757`），系统将从 SQLite / 审计转录中自动提取遗留痛点并召集专家会诊。
- **三元认知分工与 Jev 裁决**:
  - **Mark (Antigravity)**: 战略综合与架构规划。
  - **Kat (Hermes)**: 经验实证、服务器守护进程与端到端测试。
  - **Nika (OpenCode)**: 局部代码手术、风险审计与逻辑反思。
  - **Jev (TypeSafe System One)**: 独立客观裁决人，以严格的标准验收成果。
- **概念替换防范机制**: 严格禁止单一智能体用内部子智能体替代三方独立多模型会诊，确保认知多样性真实有效。
